...
async def get_model_config():
    ...
    try:
        ...
        for provider_id, provider_config in configs["providers"].items():
            ...
            providers.append(
                Provider(
                    id=provider_id, name=f"{provider_id.capitalize()}", supportsCustomModel=provider_config.get("supportsCustomModel", False), models=models
                )
            )
        ...
        return ModelConfig(
            providers=[Provider(id="google", name="Google", supportsCustomModel=True, models=[Model(id="gemini-2.0-flash", name="Gemini 2.0 Flash")])],
...
async def export_wiki(request: WikiExportRequest):
    ...
    try:
        ...
        response = Response(content=content, media_type=media_type, headers={"Content-Disposition": f"attachment; filename={filename}"})
...
async def get_local_repo_structure(path: str = Query(None, description="Path to local repository")):
    ...
    try:
        ...
        return JSONResponse(status_code=500, content={"error": f"Error processing local repository: {str(e)}"})
...
def generate_json_export(repo_url: str, pages: List[WikiPage]) -> str:
    ...
    export_data = {
        "metadata": {"repository": repo_url, "generated_at": datetime.now().isoformat(), "page_count": len(pages)},
...
async def save_wiki_cache(data: WikiCacheRequest) -> bool:
    ...
    try:
        payload = WikiCacheData(wiki_structure=data.wiki_structure, generated_pages=data.generated_pages)
