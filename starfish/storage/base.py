...
from typing import Any, List, Dict, Optional, Union, Type, Set, Tuple
...
from starfish.storage.models import (
    ...
    StorageCapabilities, RecordSchema, AssociationSchema
)
...
class RecordTypeRegistry:
    ...
    def register(cls, name: str, schema: Dict, version: int = 1, description: Optional[str] = None) -> None:
        ...
        cls._types[name] = RecordSchema(
            name=name,
            version=version,
            schema=schema,
            description=description or f"Record type: {name}"
        )
...
class AssociationTypeRegistry:
    ...
    def register(cls, name: str, schema: Dict, version: int = 1, description: Optional[str] = None) -> None:
        ...
        cls._types[name] = AssociationSchema(
            name=name,
            version=version,
            schema=schema,
            description=description or f"Association type: {name}"
        )
