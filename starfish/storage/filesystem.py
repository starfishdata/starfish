...
class FileSystemStorage(Storage):
    ...
    capabilities = StorageCapabilities(supports_queries=False, supports_projects=True, supports_associations=True, supports_paging=True, max_batch_size=10000)
    ...
    def _create_path_in_default_project(self, job_id: str) -> Path:
        ...
        if not self.load_project(default_project_id):
            project = Project(
                project_id=default_project_id, name="Auto-created Project", description="Project automatically created for orphaned jobs", created_by="system"
            )
    ...
    def save_project(self, project: Project) -> Dict[str, Any]:
        ...
        return {"project_id": project.project_id, "path": str(path)}
    ...
    def save_job_metadata(self, metadata: JobMetadata) -> Dict[str, Any]:
        ...
        return {"job_id": job_id, "project_id": metadata.project_id, "parent_job_id": parent_job_id, "path": str(path)}
    ...
    def load_record(self, record_id: str) -> Optional[Record]:
        ...
        for project in self.list_projects():
            ...
            record_patterns = [f"*/data_*.{self._get_file_extension()}", f"*/*/data_*.{self._get_file_extension()}"]
    ...
    def save_associations(self, associations: List[RecordAssociation], job_id: str) -> Dict[str, Any]:
        ...
        return {"job_id": job_id, "project_id": project_id, "parent_job_id": parent_job_id, "filepath": str(path), "association_count": len(associations)}
                    ...
                    with open(path, "r", encoding="utf-8") as f:
                        for line in f:
                            if line.strip():
                                ...
                                if (
                                    (source_record_id is None or association.source_record_id == source_record_id)
                                    and (target_record_id is None or association.target_record_id == target_record_id)
                                    and (association_type is None or association.association_type == association_type)
                                ):
...
def create_filesystem_storage(config: FileSystemStorageConfig) -> Storage:
    ...
    logger.debug("📁 STORAGE INITIALIZED: filesystem")
