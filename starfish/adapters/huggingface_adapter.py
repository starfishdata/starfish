...
It does not directly interact with model serving or hosting.
"""
import os
import aiohttp
import asyncio
...
async def list_hf_models(query: str = "", limit: int = 20) -> List[Dict[str, Any]]:
    ...
    if success:
        ...
        return [
            ...
            or any(tag in model.get("tags", []) for tag in ["llm", "gguf", "quantized"])
        ]
    else:
...
async def get_imported_hf_models() -> List[str]:
    ...
    """
    from starfish.adapters.ollama_adapter import list_models
    
    models = await list_models()
    return [
        model.get("name", "")
        for model in models
        if model.get("name", "").startswith("hf-")
    ]
...
async def find_gguf_files(model_id: str) -> List[Dict[str, Any]]:
    ...
    gguf_files = [
        ...
        file.get("path", "").lower().endswith(".gguf")
    ]
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers) as response:
                ...
                logger.info(f"Download started: 0.0%")
                    ...
                    async for chunk in response.content.iter_chunked(chunk_size):
                        ...
                        if total_size:
                            ...
                            if (current_time - last_progress_time >= 5 or 
                                (current_percentage % 25 == 0 and current_percentage != last_percentage)):
...
async def ensure_hf_model_ready(model_id: str) -> Tuple[bool, str]:
    ...
    Raises:
        ...
        HuggingFaceError: For other HuggingFace-related errors
    """
    from starfish.adapters.ollama_adapter import ensure_model_ready, OllamaConnectionError
