...
async def generate_topics(
    ...
    for _ in range(num_batches):
        ...
        input_params = {
            ...
            'num_records': min(llm_batch_size, num_topics - len(generated_topics))
        }
...
if __name__ == "__main__":
    ...
    result1 = asyncio.run(prepare_topic(
        topics=topics1, 
        num_records=100, 
        records_per_topic=25, 
        user_instructions="some context"
    ))
    ...
    result2 = asyncio.run(prepare_topic(
        topics=topics2, 
        num_records=10
    ))
    ...
    result3 = asyncio.run(prepare_topic(
        topics=topics3, 
        num_records=30, 
        user_instructions="mixed topics"
    ))
    ...
    async def run_example5():
        result = await prepare_topic(
            topics=None,
            num_records=10,
            records_per_topic=5,
            user_instructions="cloud computing"
        )
