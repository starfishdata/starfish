...
class JSONParser:
    ...
    @staticmethod
    def get_format_instructions(schema: Dict[str, Any], json_wrapper_key: Optional[str] = None, 
                                 show_array_items: int = 1) -> str:
        """
        ...
        def format_property(name: str, prop: Dict[str, Any], required: List[str], indent_level: int = 1) -> List[str]:
            ...
            if field_type == "object" and "properties" in prop:
                ...
                if items.get("type") == "object" and "properties" in items:
                    ...
                    for i, (nested_name, nested_prop) in enumerate(nested_props.items()):
                        example_value = '""' if nested_prop.get("type") == "string" else \
                                      "0" if nested_prop.get("type") in ["integer", "number"] else \
                                      "false" if nested_prop.get("type") == "boolean" else \
                                      "[]" if nested_prop.get("type") == "array" else "{}"
                        
                        nested_comment = f"// {nested_prop.get('description', '')}" + \
                                       (" (required)" if nested_name in nested_required else " (optional)")
                ...
                example_value = '""' if field_type == "string" else \
                              "0" if field_type in ["integer", "number"] else \
            ...
            if type_check:
                for field_name, field_schema in properties.items():
                    ...
                    if field_value is None:
                        if field_schema.get("type") != "null" and "null" not in field_schema.get("type", []):
                            validation_errors.append(
                                f"Item {index}: Field '{field_name}' is null but type should be {field_schema['type']}"
                            )
                        continue
                    ...
                    if expected_python_type and not isinstance(field_value, expected_python_type):
                        validation_errors.append(
                            f"Item {index}: Field '{field_name}' has type {type(field_value).__name__} "
                        ...
                        if items_schema.get("type") == "object":
                            ...
                            for array_idx, array_item in enumerate(field_value):
                                if not isinstance(array_item, dict):
                                    validation_errors.append(
                                        f"Item {index}: Field '{field_name}[{array_idx}]' should be an object"
                                    )
                                    continue
                                ...
                                try:
                                    ...
                                    for error in e.details["errors"]:
                                        validation_errors.append(
                                            error.replace("Item 0:", f"Item {index}: Field '{field_name}[{array_idx}]'")
                                        )
    ...
    @staticmethod
    def parse_llm_output(text: str, schema: Optional[Dict[str, Any]] = None, 
                      json_wrapper_key: Optional[str] = None,
                      strict: bool = False, type_check: bool = False) -> Optional[Any]:
        """
