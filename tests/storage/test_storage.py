...
def test_storage() -> Storage:
    ...
    config = FileSystemStorageConfig(type="filesystem", base_path="test_data_results", data_format="jsonl")
...
def test_project() -> Project:
    ...
    return Project(
        ...
        parameters={"model_name": "test_model", "temperature": 0.7},
    )
...
def test_job_metadata(test_project) -> JobMetadata:
    ...
    return JobMetadata(
        ...
        step_states={"batch_splitter": {"status": "completed"}, "generate_data": {"status": "completed"}},
    )
...
def test_evaluation_job(test_project, test_records) -> JobMetadata:
    ...
    return JobMetadata(
        ...
        parameters={"metrics": ["accuracy", "relevance"], "model_name": "test_eval_model"},
    )
...
def test_associations(test_records, test_evaluation_job) -> List[RecordAssociation]:
    ...
    for record in test_records:
        association = RecordAssociation(
            ...
            metadata={"evaluator": "test_model", "evaluation_time": datetime.now().isoformat()},
        )
...
def test_record_operations(test_storage, test_project, test_job_metadata, test_records):
    ...
    project_gen_records = test_storage.load_records_by_project(test_job_metadata.project_id, job_type="generation")
...
def test_backward_compatibility(test_storage, test_project):
    ...
    job_metadata = JobMetadata(
        ...
        job_type="generation",
        pipeline_name="test_pipeline",
        parameters={"model_name": "test_model", "instruction": "Test instruction"},
    )
    ...
    records = [
        Record(
            job_id=job_id,
            record_type="qa_pair",
            data={"question": "Test compatibility question?", "answer": "Test compatibility answer."},
            metadata={"topic": "test_topic", "model_name": "test_model"},
        )
    ]
...
def test_complete_workflow(test_storage):
    ...
    # 1. Create a project
    project = Project(name="Integration Test Project", description="A project for testing the complete workflow", created_by="integration_test")
    ...
    gen_job = JobMetadata(
        ...
        parameters={"instruction": "Generate integration test data", "model_name": "integration_model", "num_records": 5},
    )
    ...
    for i in range(5):
        record = Record(
            ...
            data={"field1": f"Value {i}", "field2": i * 10, "field3": {"nested": f"nested_value_{i}"}},
            metadata={"source": "integration_test", "batch_index": i},
        )
        records.append(record)
    ...
    eval_job = JobMetadata(
        project_id=project_id, job_type="evaluation", status="pending", pipeline_name="evaluation_pipeline", parameters={"eval_metric": "quality"}
    )
    ...
    for record in records:
        assoc = RecordAssociation(
            ...
            job_id=eval_job_id,
            scores={"quality": round(0.5 + 0.5 * (hash(record.record_id) % 10) / 10, 2)},
        )
