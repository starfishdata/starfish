...
async def test_step1(config, state):
    ...
    for i in range(num_results):
        results.append({"id": f"result_{i}", "text": f"{input_text} - {i}"})
...
async def test_step2(config, state):
    ...
    for result in results:
        processed_results.append({"id": result["id"], "text": result["text"].upper(), "length": len(result["text"])})
...
class TestPipelineWithStorage(unittest.TestCase):
    ...
    async def test_pipeline_with_storage(self):
        ...
        pipeline = Pipeline(
            ...
            steps=[{"name": "test_step1", "config": {"type": "test"}}, {"name": "test_step2", "config": {"type": "test"}}],
            storage_config={"type": "filesystem", "base_path": self.storage_path, "data_format": "jsonl", "project_name": "Test Project"},
        )

        # Run the pipeline
        result = await pipeline.run_async({"input_text": "Hello, world", "output_path": os.path.join(self.test_dir, "output.json"), "num_results": 5})
    ...
    async def test_pipeline_with_step_failure(self):
        ...
        pipeline = Pipeline(
            ...
            storage_config={"type": "filesystem", "base_path": self.storage_path, "data_format": "jsonl", "project_name": "Failure Test"},
        )

        # Run the pipeline and expect it to fail
        with self.assertRaises(ValueError):
            await pipeline.run_async({"input_text": "Hello, world", "output_path": os.path.join(self.test_dir, "output.json"), "num_results": 5})
