...
async def test_basic_workflow():
    ...
    try:
        ...
        print(f"  - Updated master job status to: running")
        ...
        job.start_time = now
        await storage.log_execution_job_start(job)
        print(f"  - Updated execution job status to: running")
        ...
        for i, record in enumerate(records):
            ...
            output_ref = await storage.save_record_data(
                record.record_uid, master_job_id, job_id, data
            )
        ...
        print(f"  - Marked execution job as completed")
        ...
        print(f"  - Marked master job as completed")
...
async def small_performance_test():
    ...
    try:
        ...
        master_job_id = str(uuid.uuid4())
        config_data = {
            "generator": "perf_test_generator",
            "parameters": {"num_records": TOTAL_RECORDS}
        }
        config_ref = await storage.save_request_config(master_job_id, config_data)
        ...
        for job_idx in range(NUM_JOBS):
            ...
            for rec_idx in range(RECORDS_PER_JOB):
                ...
                output_ref = await storage.save_record_data(
                    record_uid, master_job_id, job_id, data
                )
