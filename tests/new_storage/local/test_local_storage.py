...
async def test_master_job_lifecycle(storage, test_project):
    ...
    await storage.log_master_job_end(
        master_job_id, "completed", summary, end_time, end_time
    )
...
async def test_execution_job_lifecycle(storage, test_master_job):
    ...
    await storage.log_execution_job_end(
        job_id, "completed", counts, now, now
    )
...
async def test_list_execution_jobs(storage, test_master_job):
    ...
    # Filter by status
    completed = await storage.list_execution_jobs(
        test_master_job.master_job_id, 
        status_filter=["completed"]
    )
    assert len(completed) == 4
    
    # Multiple status
    multi_status = await storage.list_execution_jobs(
        test_master_job.master_job_id,
...
async def test_record_storage(storage, test_master_job):
    ...
    # Save record data
    output_ref = await storage.save_record_data(
        record_uid, test_master_job.master_job_id, job.job_id, record_data
    )
...
async def test_get_records_for_master_job(storage, test_master_job):
    ...
    for status, count in status_counts.items():
        for i in range(count):
            ...
            if status == "completed":
                data = {"value": i}
                output_ref = await storage.save_record_data(
                    record_uid, test_master_job.master_job_id, job.job_id, data
                )
            else:
                output_ref = None
    ...
    # Filter by status
    completed = await storage.get_records_for_master_job(
        test_master_job.master_job_id,
        status_filter=["completed"]
    )
    ...
    # Multiple status
    multi_status = await storage.get_records_for_master_job(
        test_master_job.master_job_id,
        status_filter=["failed", "filtered"]
    )
    assert len(multi_status) == 5
    ...
    filtered_counts = await storage.count_records_for_master_job(
        test_master_job.master_job_id,
...
async def test_complete_workflow(storage):
    ...
    for i in range(10):
        ...
        if i < 8:  # 8 successful, 2 failed
            output_ref = await storage.save_record_data(
                record_uid, master_job_id, job_id, record_data
            )
            status = "completed"
            error = None
        else:
    ...
    completed_records = await storage.get_records_for_master_job(
