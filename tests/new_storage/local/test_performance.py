...
def parse_args():
    ...
    parser.add_argument(
        "--total-records", type=int, default=DEFAULT_TOTAL_RECORDS,
        help=f"Total number of records to generate (default: {DEFAULT_TOTAL_RECORDS})"
    )
    parser.add_argument(
        "--batch-size", type=int, default=DEFAULT_BATCH_SIZE,
        help=f"Number of records per job (default: {DEFAULT_BATCH_SIZE})"
    )
    parser.add_argument(
        "--concurrency", type=int, default=DEFAULT_CONCURRENCY,
        help=f"Maximum number of concurrent operations (default: {DEFAULT_CONCURRENCY})"
    )
    parser.add_argument(
        "--complexity", choices=["small", "medium", "large"], default=DEFAULT_DATA_COMPLEXITY,
        help=f"Size/complexity of generated data (default: {DEFAULT_DATA_COMPLEXITY})"
    )
    parser.add_argument(
        "--db-dir", type=str, default=TEST_DB_DIR,
        help=f"Directory for test database (default: {TEST_DB_DIR})"
    )
    parser.add_argument(
        "--use-batch", action="store_true",
        help="Use batch operations where possible for better performance"
    )
    parser.add_argument(
        "--read-performance", action="store_true",
        help="Run the read performance test instead of the workflow test"
    )
...
def generate_qa_data(index: int, complexity: str) -> Dict[str, Any]:
    ...
    if complexity == "large":
        ...
        data["examples"] = [
            {f"example_{i}": f"This is example {i} for question {index}. " * 3}
            for i in range(10)
        ]
...
async def test_realistic_workflow(
    ...
    project = Project(
        ...
        name="QA Dataset Project",
        description="A project for generating question-answer pairs"
    )
    ...
    async def process_job(job_idx, job):
        ...
        for i, record in enumerate(job_records):
            ...
            try:
                ...
                # Normal successful generation
                    ...
                    data_save_task = storage.save_record_data(
                        record.record_uid, master_job_id, job.job_id, record_data
                    )
        ...
        await storage.log_execution_job_end(
            job.job_id, "completed", job_counts, job_end_time, job_end_time
        )
        
        job_time = time.time() - job_start_time
        print(f"  - Job {job_idx+1}/{num_jobs} completed: {job_time:.4f}s, " +
              f"{actual_batch_size} records ({job_time/actual_batch_size:.4f}s per record)")
    ...
    print(f"4. Record generation and processing: {processing_time:.4f}s " +
          f"({processing_time/total_records:.4f}s per record)")
    ...
    now = datetime.datetime.now(datetime.timezone.utc)
    await storage.log_master_job_end(
        master_job_id, "completed", summary, now, now
    )
    ...
    for page in range(min(3, page_count)):  # Test up to 3 pages
        page_tasks.append(storage.get_records_for_master_job(
            master_job_id, limit=page_size, offset=page * page_size
        ))
    ...
    completed_recs = await storage.get_records_for_master_job(
        master_job_id, status_filter=["completed"], limit=100
    )
...
async def test_read_performance():
    ...
    try:
        ...
        project = Project(
            project_id=str(uuid.uuid4()),
            name="Read Performance Test Project",
            description="Project for testing read performance"
        )
        ...
        for i in range(total_records):
            ...
            if rand_val < 0.85:  # 85% completed
                ...
                output_ref = await storage_instance.save_record_data(
                    record.record_uid, master_job_id, job_id, record_data
                )
                record.status = "completed"
        ...
        now = datetime.datetime.now(datetime.timezone.utc)
        await storage_instance.log_execution_job_end(
            job_id, "completed", job_counts, now, now
        )
        ...
        await storage_instance.log_master_job_end(
            master_job_id, "completed", summary, now, now
        )
        ...
        for iteration in range(1, iterations + 1):
            ...
            completed_records = await storage_instance.get_records_for_master_job(
                master_job_id, status_filter=["completed"]
            )
