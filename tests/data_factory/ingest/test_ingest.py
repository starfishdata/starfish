...
from src.starfish.data_factory.data_ingest.ingest import determine_parser, generate_input_data, process_file
from src.starfish.data_factory.data_ingest.parsers import (
    PDFParser,
    WordDocumentParser,
    PPTParser,
    TXTParser,
...
from starfish.data_factory.utils.token_spliter import TokenTextSplitter
from starfish.data_factory.utils.util import async_read_file
...
async def test_process_file_gina_ai():
    # gina_pdf_url = "https://www.researchgate.net/profile/Arnold-Mawaggali/publication/344469510_Autonomous_Economic_Agents_with_the_FetchAI_Open_Economic_Framework/links/5f7d0cf3299bf1b53e130f92/Autonomous-Economic-Agents-with-the-FetchAI-Open-Economic-Framework.pdf"
    # gina_pdf_url = "https://drive.google.com/drive/folders/1yvPtFq-JYCgZVVcv_gg63PzcjCw2dbpp"
...
async def test_ingest_input_data():
    ...
    all_messages = generate_input_data(content)

    result = await test_ingest_pdf.run(prompt_msg=all_messages)
    assert len(result) == 4
    # esume_from_checkpoint(043e5040-e373-4c74-8f7e-cdf0983765a1)
