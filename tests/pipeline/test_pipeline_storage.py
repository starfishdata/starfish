...
# from starfish.core.storage.models import FileSystemStorageConfig
# from starfish.core.workflow.pipeline import Pipeline, transactional_storage
...
#     processed_data = [{**item, "value": item["value"] * 2} for item in data]
...
#             steps=[{"name": "test_generate", "config": {"type": "test"}}, {"name": "test_process", "config": {"type": "test"}}],
#             storage_config={"type": "filesystem", "base_path": self.TEST_DIR, "data_format": "jsonl"},
#         )

#         # Run pipeline
#         result = await pipeline.run_async({"project_name": "Storage Test Project"})

#         # Check that pipeline executed successfully
#         assert result["status"] == "success"

#         # Check that storage files were created
#         storage = FileSystemStorage(FileSystemStorageConfig(type="filesystem", base_path=self.TEST_DIR, data_format="jsonl"))
...
#             context_model=TestPipelineContext,
#             steps=[{"name": "test_generate", "config": {"type": "test"}}, {"name": "test_process", "config": {"type": "test"}}],
#             storage_config={"type": "filesystem", "base_path": self.TEST_DIR, "data_format": "jsonl", "enabled": False},
#         )

#         # Run pipeline
#         result = await pipeline.run_async({"project_name": "Disabled Storage Test"})
...
#             context_model=TestPipelineContext,
#             steps=[{"name": "test_generate", "config": {"type": "test"}}, {"name": "test_process", "config": {"type": "test"}}],
#             storage_config={"type": "filesystem", "base_path": "wrong_directory", "data_format": "jsonl"},
#         )

#         # Run pipeline with overridden storage config
#         result = await pipeline.run_async({"project_name": "Override Storage Test", "storage.base_path": self.TEST_DIR})
