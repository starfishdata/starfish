...
from starfish.core.structured_llm import get_structured_llm, StructuredLLM, LLMResponse
from starfish.common.utils import to_sync
from starfish.common.prompt import PromptManager, get_prompt, get_partial_prompt
...
def test_run_sync_method(sample_json_schema, sample_prompt):
    ...
    with patch.object(builder, "run") as mock_run:
        ...
        mock_response = LLMResponse(
            raw_response=MockResponse('{"question": "Test?", "answer": "Answer"}'),
            parsed_data={"question": "Test?", "answer": "Answer"}
        )
    ...
    with patch("starfish.services.structured_llm.call_chat_model", new_callable=AsyncMock) as mock_call_chat:
        with patch("starfish.common.prompt.get_prompt") as mock_get_prompt:
            ...
            response = await builder.run(
                user_instruction="Generate Q&A pairs about history",
                num_records=2,
                schema_instruction="JSON format with question and answer fields"
            )
