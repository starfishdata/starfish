"""Data Factory Module - Core implementation for data processing pipelines.

This module provides the @data_factory decorator and Factory class for creating
and managing data generation pipelines with features like batch processing,
async execution, resume capability, and progress tracking.
"""
...
from starfish.data_factory.config import NOT_COMPLETED_THRESHOLD, TASK_RUNNER_TIMEOUT, PROGRESS_LOG_INTERVAL
...
class Factory:
    ...
    def _clean_up_in_same_session(self):
        """Reset state for same session reuse."""
    ...
    def _check_parameter_match(self):
        ...
        Raises:
            InputError: If there's a mismatch between input data parameters and function parameters
        """
...
def resume_from_checkpoint(*args, **kwargs) -> List[dict[str, Any]]:
    """Resume a previously interrupted data processing job.
