...
async def generate_country_info(cities: List[str], num_facts: int, llms: List[StructuredLLM]) :
    """Sample workflow processing cities in batches"""
    cities_info = []
    for city in cities:
        model_response = await llms[0].run(city_name=city)
...
results = manager.execute_with_retry(generate_country_info, cities=cities, num_facts=3, llms=[first_llm, validation_llm])
